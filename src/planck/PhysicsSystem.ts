...
import { Body, BoxShape, ChainShape, CircleShape, Contact, ContactImpulse, EdgeShape, Fixture, Manifold, PolygonShape, World } from 'planck'
import { color, director, DrawNode, p, Vec2 } from 'safex-webgl'
...
export class PhysicsSystem implements System {
  ...
  configure(event_manager: EventManager) {
    ...
    event_manager.subscribe(EventTypes.ComponentAdded, RigidBody, ({ entity, component: rigidBody }) => {
      ...
      const zero = Vec2(0, 0)
      const position = Vec2(node.posX / PTM_RATIO, node.posY / PTM_RATIO)
        ...
        if (comp.node?.active && comp.enabled) {
          ...
          if (this.debugNode) {
            ...
            if (shape instanceof BoxShape || shape instanceof PolygonShape) {
              ...
              this.debugNode.drawCircle(toPixels(pos), radius, 0, 20, false, 1, color(255, 0, 0, 255))
              ...
              this.debugNode.drawSegment(toPixels(v1), toPixels(v2), 1, color(255, 255, 255, 255))
              ...
              for (let i = 0; i < vertices.length - 1; i++) {
                ...
                this.debugNode.drawSegment(toPixels(v1), toPixels(v2), 1, color(200, 200, 200, 255))
              }
            }
          }
        }
  ...
  contactBegin(contact: Contact) {
    ...
    if (phys1 && phys2) {
      if (Object.prototype.hasOwnProperty.call(phys1.props, 'onBeginContact')) {
        phys1.props.onBeginContact(phys2, contact)
      }
      if (Object.prototype.hasOwnProperty.call(phys2.props, 'onBeginContact')) {
        phys2.props.onBeginContact(phys1, contact)
      }
    }
  }
  ...
  set gravity(val: Vec2) {
    this.world.setGravity(Vec2(val.x, val.y))
